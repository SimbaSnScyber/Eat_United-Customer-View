...
if(isset($_GET['prodExtra1Rem']) || isset($_GET['prodExtra2Rem']) || isset($_GET['prodExtra3Rem'])){
	...
	header("Refresh:0; url=resturant.php?restId= $restID");
}
		...
		while ($row=mysqli_fetch_array($cartProducts)) {
			...
			if($extrasel){
				if(strcasecmp($extrasel,$extraName1) == 0){
					echo '<tr>
				  <td style="text-align:left; "><a href="resturant.php?restId='.$restID.'action=remove&code='.$rowID.'&prodExtra1Rem='.$extraName1.'&prodExtra2Rem='.$extraName2.'&prodExtra3Rem='.$extraName3.'" class="btnRemoveAction"><img src="img/minus-sign.png" height="18px" alt=" " /></a></td>
...
?>
								...
								<ul class="clearfix">
									<br>
									<li><a id="btnEmpty" href="resturant.php?restId=<?php echo $restID ; ?>&action=empty&empty=true">Empty Cart</a></li>

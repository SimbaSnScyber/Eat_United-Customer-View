...
if(isset($_GET['prodExtra1Rem']) || isset($_GET['prodExtra2Rem'])){
	...
	header("Refresh:0; url=Top-Restaurant .php");
}
...
<body data-spy="scroll" data-target="#secondary_nav" data-offset="75">
	        ...
	        <div class="row">
	            <div class="col-lg-8">
	                <div class="detail_page_head clearfix">
	                    <div class="rating">
						<?php
	$product= mysqli_query($conn,"SELECT FLOOR(AVG(average)) as averages, COUNT(id) as ratings FROM reviews WHERE restaurant =  'Top Restaurant and Bar' ORDER BY id ASC");
	if (!empty($product)) { 
	...
	?> </div>
	                    <div class="title">
						<?php
	$product= mysqli_query($conn,"SELECT * FROM restaraunt WHERE id = '3002'");
	if (!empty($product)) { 
		while ($row=mysqli_fetch_array($product)) {
	                    ...
	                    <section id="section-1">
	                        ...
	                        <div class="table_wrapper">
								...
								<thead>
	                                ...
	                                <tbody>
									<?php
	$product= mysqli_query($conn,"SELECT * FROM products WHERE category = 'main' && restID = '3002' ORDER BY id ASC");
	if (!empty($product)) {
...
$cartProducts= mysqli_query($conn,"SELECT * FROM current_order WHERE user_id =".$user_id." ORDER BY id ASC");
	if (!empty($cartProducts)) { 
		while ($row=mysqli_fetch_array($cartProducts)) {
			...
			if($extrasel){
				if(strcasecmp($extrasel,$extraName1) == 0){
					echo '<tr>
				  <td style="text-align:left; "><a href="Top-Restaurant .php?action=remove&code='.$rowID.'&prodExtra1Rem='.$extraName1.'&prodExtra2Rem='.$extraName2.'" class="btnRemoveAction"><img src="img/minus-sign.png" height="18px" alt=" " /></a></td>
		...
		?>
								...
								<ul class="clearfix">
									<br>
									<li><a id="btnEmpty" href="Top-Restaurant .php?action=empty&empty=true">Empty Cart</a></li>
...
?>
								...
								<input type="hidden" name="restName" id="restName" value="Top Restaurant and Bar"/>
									</form>
	        ...
	        <div class="row">
	            <div class="col-lg-8 list_menu">
	                <section id="section-5">
	                    <h4>Reviews</h4>
						<?php
	$product= mysqli_query($conn,"SELECT * FROM reviews WHERE restaurant = 'Top Restaurant and Bar' ORDER BY id ASC");
	if (!empty($product)) { 
		while ($row=mysqli_fetch_array($product)) {

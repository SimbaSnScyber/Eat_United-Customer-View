...
if(isset($_GET['prodExtra1Rem']) || isset($_GET['prodExtra2Rem'])){
	...
	header("Refresh:0; url=Tafule-Yaka .php");
}
...
<body data-spy="scroll" data-target="#secondary_nav" data-offset="75">
	        ...
	        <div class="row">
	            <div class="col-lg-8">
	                <div class="detail_page_head clearfix">
	                    <div class="rating">
						<?php
	$product= mysqli_query($conn,"SELECT FLOOR(AVG(average)) as averages, COUNT(id) as ratings FROM reviews WHERE restaurant =  'Tafule Yaka Restaurant & Bar' ORDER BY id ASC");
	if (!empty($product)) { 
	...
	?>  </div>
	                    <div class="title">
						<?php
	$product= mysqli_query($conn,"SELECT * FROM restaraunt WHERE id = '3005'");
	if (!empty($product)) { 
		while ($row=mysqli_fetch_array($product)) {
	...
	<?php echo $row["address"]; ?> - <a href="<?php echo $row["maps"]; ?>" target="blank">Get directions</a>
		...
		?>
	                ...
	                <!-- /detail_page_head -->
					<br>
	                <h6>About "Tafule Yaka Restaurant & Bar"</h6>
	                <p>Situated in the Zambezi region at the Protea Hotel by Marriott Zambezi River Lodge, Tafule Yaka provides its customers with a breathtaking view of the Zambezi River and a delectable menu which consists of a variety of African Cuisine. Our Bar offers our customers a variety of beverages, which can be slowly sipped whilst you take in the beautiful view of the surrounding area.</p>
	            </div>
	            <div class="col-lg-4">
	                <div class="pictures magnific-gallery clearfix">
					<figure>
	                        <a href="https://eatatunited.com.na/wp-content/uploads/2020/08/Tafule-yaka-bg.jpg" title="Photo title" data-effect="mfp-zoom-in">
	                            <img src="https://eatatunited.com.na/wp-content/uploads/2020/08/Tafule-yaka-bg.jpg" class="lazy loaded" alt="" data-was-processed="true">
	                        </a>
	                    </figure>
	                    ...
	                    <section id="section-1">
	                        ...
	                        <div class="table_wrapper">
								...
								<thead>
	                                ...
	                                <tbody>
									<?php
	$product= mysqli_query($conn,"SELECT * FROM products WHERE category = 'main' && restID = '3005' ORDER BY id ASC");
	if (!empty($product)) {
...
$cartProducts= mysqli_query($conn,"SELECT * FROM current_order WHERE user_id =".$user_id." ORDER BY id ASC");
	if (!empty($cartProducts)) { 
		while ($row=mysqli_fetch_array($cartProducts)) {
			...
			if($extrasel){
				if(strcasecmp($extrasel,$extraName1) == 0){
					echo '<tr>
				  <td style="text-align:left; "><a href="Tafule-Yaka .php?action=remove&code='.$rowID.'&prodExtra1Rem='.$extraName1.'&prodExtra2Rem='.$extraName2.'" class="btnRemoveAction"><img src="img/minus-sign.png" height="18px" alt=" " /></a></td>
		...
		?>
								...
								<ul class="clearfix">
									<br>
									<li><a id="btnEmpty" href="Tafule-Yaka .php?action=empty&empty=true">Empty Cart</a></li>
...
?>
								...
								<input type="hidden" name="restName" id="restName" value="Tafule Yaka Restaurant & Bar"/>
									</form>
	        ...
	        <div class="row">
	            <div class="col-lg-8 list_menu">
	                <section id="section-5">
	                    <h4>Reviews</h4>
	                    <?php
	$product= mysqli_query($conn,"SELECT * FROM reviews WHERE restaurant = 'Tafule Yaka Restaurant & Bar' ORDER BY id ASC");
	if (!empty($product)) { 
		while ($row=mysqli_fetch_array($product)) {
	                    ...
	                    <div class="row add_bottom_30 d-flex align-items-center reviews">
	                        <div class="col-md-3">
	                            <div id="review_summary">
	                                <em>Tafule Yaka Restaurant & Bar</em>
	                            </div>
	                        </div>
